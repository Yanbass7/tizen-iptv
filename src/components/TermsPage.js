...
const TermsPage = ({ onAccept }) => {
  ...
  // Exibição do popup é controlada pelo AlertPopup (auto-hide em 3s)
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (code === 13) { // Enter
      ...
      if (activeButton === 'accept') {
        ...
        console.log('Botão Rejeitar acionado (Termos)');
        triggerRejectNotice();
      }
      return;
    }
  }, [activeButton, onAccept, triggerRejectNotice]);

  useEffect(() => {
    const listener = (ev) => handleKeyDown(ev);
    window.addEventListener('termsNavigation', listener);
    // Também ouvir keydown global para garantir suporte em todas as TVs/navegadores
    document.addEventListener('keydown', listener);
    return () => {
      window.removeEventListener('termsNavigation', listener);
      ...
      <div className="terms-modal">
        ...
        <div className="terms-actions">
          <button
            ...
            type="button"
            onClick={() => { console.log('Botão Rejeitar clicado (Termos)'); triggerRejectNotice(); }}
            onKeyDown={(e) => {
              if (e.keyCode === 39) { // Right
                e.preventDefault();
                setActiveButton('accept');
              } else if (e.keyCode === 13) {
                console.log('Botão Rejeitar acionado via Enter (Termos)');
                triggerRejectNotice();
              }
            }}
          >
